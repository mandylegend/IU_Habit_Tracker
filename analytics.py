...
def longest_streak(completions, periodicity):
    ...
    dates = sorted([datetime.strptime(c, "%Y-%m-%d %H:%M:%S")
...
def habit_data_graph(habit_id):
    ...
    df = df.groupby(df["completion_date"].dt.date).size(
    ).reset_index(name='count')

    fig = px.bar(df, x="completion_date", y="count", title="Habit Completion History", labels={
                 "completion_date": "Date", "count": "Completions"})
    return fig


def hit_rate(completions, total_days):
    if total_days == 0:
        return 0.0
    else:
        return (len(completions) / total_days) * 100


def skip_days(completions, periodicity):
    if not completions:
        return 0
